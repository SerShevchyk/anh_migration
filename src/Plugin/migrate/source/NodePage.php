 ...
 * )
 */
class NodePage extends SqlBase {
  ...
  public function fields() {
    $fields = [
      ...
      // Custom added fields.
      'field_page_slide_image' => $this->t('Slide image'),
      'field_page_slide_summary' => $this->t('Slide summary'),
      'field_page_body' => $this->t('Body'),
      'field_page_right_column_content' => $this->t('Right column content'),
      'field_page_file_attachments' => $this->t('File attachments'),
      'field_page_components' => $this->t('Components'),
      'field_academy' => $this->t('Academy'),
      'field_page_type' => $this->t('Type'),
    ];
    return $fields;
  }
  ...
  public function prepareRow(Row $row) {
    ...
    $query = $this->select('field_data_field_slide_image', 'fdfsi')
      ...
      ->execute()
      ->fetch();
    $row->setSourceProperty('field_page_slide_image', $query['field_slide_image_fid']);

    // field_page_slide_summary
    $query = $this->select('field_data_field_slide_summary', 'fdfss')
      ->fields('fdfss', ['field_slide_summary_value'])
      ->condition('entity_id', $nid)
      ->execute()
      ->fetch();
    $row->setSourceProperty('field_page_slide_summary', $query['field_slide_summary_value']);

    // field_page_slide_home_pages
    ...
    $row->setSourceProperty('field_page_body_value', $body);
    $row->setSourceProperty('field_page_body_summary', $query['body_summary']);
    $row->setSourceProperty('field_page_body_format', 'full_html');

    // field_page_right_column_content
    $query = $this->select('field_data_field_right_column_content', 'fdfrcc')
      ->fields('fdfrcc', ['field_right_column_content_value'])
      ->condition('entity_id', $nid)
      ->execute()
      ->fetch();
    $row->setSourceProperty('field_page_right_column_content_value', $query['field_right_column_content_value']);
    $row->setSourceProperty('field_page_right_column_content_format', 'full_html');
    ...
    $query = $this->select('field_data_upload', 'fdu')
      ...
      ->execute()
      ->fetchCol();
    $row->setSourceProperty('field_page_file_attachments', $query);
    ...
    $query = $this->select('menu_links', 'ml')
      ...
      ->fetch();
    if ($query['plid'] == 3229) {
      $row->setSourceProperty('field_page_type', 'Working Groups');
    }
    ...
    $row->setSourceProperty('field_page_components', $result);
